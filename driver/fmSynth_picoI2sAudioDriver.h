...
class fmSynthPicoI2s : public PicoI2sAudio<fmSynthPicoI2s>, public PlayTune<fmSynthPicoI2s> {
    public:
        ...
        inline void selectInstrumentAllChan(std::string patchName);
...
inline void fmSynthPicoI2s::selectInstrumentAllChan(std::string patchName)
{
    for(uint8_t i = 0; i < MAX_FM_CHANNELS; i++)
    {
        fmc[i].setChannelInstrument(patchName);
    }
}
#endif